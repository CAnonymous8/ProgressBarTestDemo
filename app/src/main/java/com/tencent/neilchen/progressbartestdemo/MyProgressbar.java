...
/**
 * Created by neil.chen on 2017/6/9.
 */
...
public class MyProgressbar extends View {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    progressPaint.setColor(Color.parseColor("#D47B7E"));
    ...
    //计算文字高度
    textPaint.setColor(Color.parseColor("#D47B7E"));
    ...
    //text = "可用内存充足";
    textLen = textPaint.measureText(number);
    textPaint.setColor(Color.parseColor("#823736"));
    textPaint.getTextBounds(text, 0, text.length(), textBounds);
    float h2 = textBounds.height();
    canvas.drawText(number, centerX - textLen / 2, centerY + h1 / 2 + h2, textPaint);
  ...
  public void setProgress(float progress) {
    ...
    new Thread(new Runnable() {
      @Override
      public void run() {
        float oldProgress = MyProgressbar.this.progress;
        for (int i = 1; i <= 100; i++) {
          MyProgressbar.this.progress = oldProgress + (validProgress - oldProgress) * (1.0f * i / 100);
          postInvalidate();
